<?php
$data = array('eng', 'host', 'user', 'pass', 'dbs', 'penc', 'port', 'char', 'url', 'lang');
...
function auto_conn($SIDU, $data) {
    if (!$SIDU[0] || !isset($SIDU['conn'][$SIDU[0]]) || !$data['id'] || $data['cmd'] == 'close') return;
...
function main_form($SIDU, $data, $err) {
    ...
    $arr_eng = array('mysql'=>'PDO MySQL', 'pgsql'=>'PDO PostgreSQL', 'sqlite'=>'PDO SQLite');
    if (!isset($arr_eng[$data['eng']])) $data['eng'] = 'mysql';
    if (!$data['user']) {
        $data['user'] = $data['eng'] == 'mysql' ? 'root' : ($data['eng'] == 'pgsql' ? 'postgres' : 'dba');
    } elseif (!$data['cmd']) {
        if (($data['user'] == 'root' || $data['user'] == 'dba') && $data['eng'] == 'pgsql') $data['user'] = 'postgres';
        elseif (($data['user'] == 'postgres' || $data['user'] == 'dba') && $data['eng'] == 'mysql') $data['user'] = 'root';
        elseif (($data['user'] == 'postgres' || $data['user'] =='root') && $data['eng'] == 'cubrid') $data['user'] = 'dba';
    }
    if (!$data['cmd']) {
        if ($data['eng'] == 'cubrid' && !$data['dbs']) $data['dbs'] = 'demodb';
        elseif ($data['eng'] != 'cubrid' && $data['dbs'] == 'demodb') $data['dbs'] = '';
        $data['penc'] = 1;
    }
    echo cms_form('form', '', '', array('class'=>'box', 'style'=>'max-width:300px;margin:15px auto', 'id'=>'conn')),
    html_logo(), ($err ? '<p class="err">'. $err .'</p>' : '');
    echo '<p class="ac"><b>SIDU '. $SIDU['sidu_ver'] .'</b> Database Web GUI</p><p class="ac">';
    $lang = array('en' => 'English', 'cn' => '中文');
    $url = ' - <a href="?'. ($SIDU[0] ? 'id='. $SIDU[0] .'&' : '') . 'lang=';
    foreach ($lang as $k => $v) echo $url, $k,'"', ($k == $SIDU['page']['lang'] ? ' class="on"' : ''), '">', $v,'</a>';
    echo '</p>', cms_form('select', 'eng',  $data['eng'],  array('list'=>$arr_eng, 'defa'=>-1));
    if ($data['eng'] != 'sqlite') {
        echo cms_form('text', 'host', $data['host'], array('placeholder'=>lang(1109).' *')),
        cms_form('text', 'user', $data['user'], array('placeholder'=>lang(1110).' *')),
        cms_form('pass', 'pass', $data['pass'], array('placeholder'=>lang(1111), 'id'=>'pwd')),
        cms_form('text', 'port', $data['port'], array('placeholder'=>lang(1112), 'style'=>'width:60px')),
        cms_form('cbox', 'penc', $data['penc'], array('list'=>lang(1113), 'id'=>'enc'));
    }
    echo cms_form('text', 'dbs',  $data['dbs'],  array('placeholder'=>'database1;db2;db3'.($data['eng'] == 'mysql' || $data['eng'] == 'pgsql' ? '' : ' *'))),
    ($data['eng'] == 'mysql' ? cms_form('select', 'char', $data['char'], array('list'=>get_conn_char(), 'defa'=>-1)) : ''),
    cms_form('submit','x', lang(1104), array('id'=>'enc_pwd')),
    cms_form('hidden','cmd', 'connect', array('id'=>'cmd')),
    cms_form('hidden','url', $data['url']),
    cms_form('hidden','id',  $data['id']),
    cms_form('hidden','lang',$SIDU['page']['lang']);
    echo '<p class="ac"><br><b class="xwin hand green" data-url="http://topnew.net/sidu">topnew.net/sidu</b></p>';
    echo cms_form('end');
    echo NL .'<br><div class="load ac" data-url="sidu_news.php?cms=SIDU'. $SIDU['sidu_ver'] .'"></div>';
...
function test_conn(&$data) {
    ...
    $data['dbs'] = strtr($data['dbs'], array(' '=>'', ','=>';', '%%'=>'%', '%%%'=>'%', '%%%%'=>'%'));//if you keep play with % i do not care
    ...
    $data['dbs'] = implode(';', $db);
    $err = '';
    $eng = $data['eng']; $host = $data['host']; $port = $data['port']; $user = $data['user']; $pass = $data['pass'];
    $pdo = $eng .':host='. $host;
    if (isset($db[0]) && $db[0]) $pdo .= ';dbname='. $db[0];
    if (($eng == 'mysql' && $port == 3306) || ($eng == 'pgsql' && $port == 5432)) $port = 0;
    elseif ($eng == 'cubrid' && !$port) $port = 30000;
    if ($port) $pdo .= ';port='. $port;
...
function main_conn($data) {
    ...
    $arr = array('eng', 'host', 'user', 'dbs', 'port', 'char');
    foreach ($arr as $k) $c[$k] = $data[$k];
    $c['pass'] = cms_enc($data['pass'], 1);
    $c['penc'] = 1;
    $c['cid']  = time() - strtotime('2016-05-05');//more then 1 user login same time same cid? not good enough fix later
