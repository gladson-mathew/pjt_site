...
function cms_clean_str($x, $trim = 0, $tag = 0, $slash = 0) {
    if (!is_array($x)) {
        if ($tag) $x = strip_tags($x, (1 == $tag ? '' : $tag));
        if ('sql' === $slash) $x = str_replace(array('#', '--', ';', '/*', '\'', '"', '\\'), '', $x);//heck free+no slash
        elseif ($slash) $x = stripslashes($x);
        if ('r' == $trim || 'R' == $trim) $x = rtrim($x);
        if ('l' == $trim || 'L' == $trim) $x = ltrim($x);
        elseif ($trim) $x = trim($x);
        return $x;
    }
    foreach ($x as $k => $v) $x[$k] = cms_clean_str($v, $trim, $tag, $slash);
    return $x;
}
function cms_firewall($ip_allowed = '', $err_msg = '') {
    if (!$ip_allowed) return;//no firewall setup
    $arr_ip = explode(';', $ip_allowed);
    if (!$arr_ip || !$arr_ip[0]) return;//ok--no firewall
    $ip = cms_ip();
    foreach ($arr_ip as $i) {
        if (preg_match($i, $ip)) return;//ok
    }
    exit($err_msg);
}
function cms_html_js($src = '', $js = '') {
    if ($src) return NL .'<script src="'. $src .'"></script>';
    if ($js) return NL .'<script>'. $js .'</script>';
}
...
function cms_ip($int = 0) {
    $ip = isset($_SERVER['HTTP_X_REMOTE_ADDR']) ? $_SERVER['HTTP_X_REMOTE_ADDR'] : (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 0);
    if (!$int) return $ip;//eg 255.2.3.4
...
function cms_pr($arr = null, $mode = '', $level = 0) {
    ...
    if (!$level) {
        echo '<span class="show green" data-src="next"><span class="show" data-src="next">'
            . ($is_OBJ ? 'object' : 'array') . ('arr' === $mode && !$is_OBJ ? '(' : '{')
            .'</span><span>'. $num . '</span></span><span class="hide">';
    }
    ...
    foreach ($arr as $k => $v) {
        ...
        echo "\n" . $tab . ($is_arr || $is_obj ? '<span class="show green" data-src="next"><span class="show" data-src="next">' : '');
        if ('arr' === $mode) echo (is_numeric($k) && ceil($k) == $k ? $k : "'$k'") .' => ';
        else echo ('json' === $mode) ? '"'. $k .'" : ' : '<b>'. $k .'</b> : ';
        if ($is_arr || $is_obj) {
            ...
            echo ('arr' === $mode ? "'" : ('json' === $mode ? '"' : ''))
                . ($len > 50 ? '<span class="show" data-src="next"><span class="show" data-src="next">' : '')
                . cms_html8(strtr(substr($v, 0, 50), $tran))
                . ($len > 50 ? '</span><i class="green">...</i></span><span class="hide">' : '')
                . cms_html8(strtr(substr($v, 50), $tran)) . ($len > 50 ? '</span>' : '')
                . ('arr' === $mode ? "'" : ('json' === $mode ? '"' : ''));
        }
        if ($mode && ++$i < $num) echo ',';
    }
...
function cms_form($type = 'form', $name = '', $data = null, $attr = null) {
    if ($type == 'end') return "\n</form>";
    if (!$type) $type = 'form';
    if ($type == 'sele') $type = 'select';
    elseif ($type == 'cbox') $type = 'checkbox';
    elseif ($type == 'pass') $type = 'password';
    $is_input = !in_array($type, array('form', 'select', 'textarea'));
    if (in_array($type, array('select', 'checkbox', 'radio'))) {
        if (!is_array($attr) || !isset($attr['list'])) $attr = array('list' => $attr);
        $list = $attr['list'];
        if (!is_array($list)) $list = array(1 => $list);
        $defa = isset($attr['defa']) ? $attr['defa'] : '';
        if (!$defa && $type == 'select' && !in_array('multiple', $attr)) {
            $defa = (isset($attr['label']) && $attr['label'] != -1) ? $attr['label'] : ucwords(str_replace('_', ' ', $name));
        }
        if ($defa && $defa != -1) {
            if (!is_array($defa)) $defa = array('' => $defa);
            $list = $defa + $list;//either merge or + both got bug -- fix later
        }
    }
    if ($is_input) $attr['type'] = $type;
    /*if (!isset($attr['placeholder']) && !in_array($type, array('', 'form', 'boot1', 'boot2', 'html', 'select', 'submit', 'reset', 'checkbox', 'radio', 'hidden'))) {
        $attr['placeholder'] = ucwords(str_replace('_', ' ', (isset($attr['label']) ? $attr['label'] : $name)));
    }*/
    if (!isset($attr['name']) && $type != 'form') {
        $attr['name'] = $name;
        if (($type == 'select ' && in_array('multiple', $attr))
         || ($type == 'checkbox' && substr($attr['name'], -1) <> ']' && is_array($attr['list']) && count($attr['list']) > 1)
        ) {
            $attr['name'] .= '[]';
        }
    }
    if ($type == 'form') {
        ...
        if (!isset($attr['method']) || !in_array(strtolower($attr['method']), array('post', 'get'))) {
            $attr['method'] = 'post';
        }
    ...
    if ($type == 'form') return $str .'>';
    if ($type == 'textarea') return $str .'>'. str_replace('</textarea>', '&lt;/textarea>', ($data ?: '')) .'</textarea>';
    if ($type == 'select') {
        $str .= '>';
        foreach ($list as $k => $v) {
            if (is_array($v)) {
                $str .= "\n" .'<optgroup label="'. cms_html8($k) .'"'. (isset($attr['class_'.$k]) ? ' class="'. $attr['class_'.$k] .'"' : '') .'>';
                foreach ($v as $k2 => $v2) $str .= cms_form_select_option($data, $attr, $k2, $v2);
                $str .= "\n</optgroup>";
            } else {
    ...
    if ($type == 'radio' || $type == 'checkbox') {
        if (!isset($attr['class_cbox'])) $attr['class_cbox'] = (count($list) > 1) ? ' ' : '';
...
function cms_form_attr($attr = null) {
    ...
    $style = '';
    $skip = array('label', 'defa', 'list', 'inline');
    foreach ($attr as $k => $v) {
        $is_int = is_numeric($k);
...
function cms_form_select_option($data, $attr, $k, $v) {
    return "\n" .'  <option value="'. cms_html8($k) .'"'. cms_form_selected($k, $data) . (isset($attr['class_'.$k]) ? ' class="'. $attr['class_'.$k] .'"' : '') .'>'. $v .'</option>';
}
function cms_form_selected($k, $val, $str = ' selected') {
    if ((is_array($val) && in_array((string)$k, $val)) || ($k == $val && strlen($k) && strlen($val))) return $str;
}
...
function sidu_list($sql = '', $para = '') {
    ...
    $arr = array();
    foreach ($rows as $r) {
        if (isset($r[0])) $arr[$r[0]] = isset($r[1]) ? $r[1] : null;
    }
    return $arr;
}
...
function sidu_keyw($keyw = '') {
    ...
    if (!$is_int && !in_array(strtoupper($keyw), $SIDU['sql_keyw']) && !$has_hyphen) return $keyw;
    if ($SIDU['eng'] != 'mysql') return '"'. $keyw .'"';
    ...
    foreach ($SIDU['rows'] as $i => $r) {
        ...
        foreach ($r as $c => $v) {
            ...
            if (($is_blob[$c] || strlen($v) > 200 || strpos($v, NL)) && (!$gridMode || substr($typ, 0, 4) != 'set(')) {
                ...
                echo $is_my_info_tab && $c == 'TABLE_NAME' ? "<a href=\"tab.php?id=$SIDU[0],$r[TABLE_SCHEMA],$SIDU[2]," . ($r['TABLE_TYPE'] == 'BASE TABLE' ? 'r' : 'v') . ",$v\">$v</a>" : nl2br(cms_html8($v));
                if (!$sql) echo '</div>';
            }
