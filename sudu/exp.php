...
$data = array('sql', 'tab', 'db', 'drop', 'desc', 'data', 'ext', 'zip', 'where', 'sepC', 'sepR', 'col', 'sql_slash', 'clean', 'null', 'csvHead');
...
function main_cout($SIDU, $mode) {
    $exp = $SIDU['data'];
    $file  = $mode == 'SQL' ? 'sidu-sql' : str_replace('/', '_', $SIDU[1]) . ($SIDU[2] ? "_$SIDU[2]" : '') . (isset($exp['sql'][1]) ? '' : '_'.$exp['tabs'][0]);
...
function main_form($SIDU, $mode) {
    ...
    $arr_ext = array('html'=>'HTML', 'csv'=>'CSV', 'sql'=>'SQL', 'json'=>'json');
    if ($mode != 'SQL') {
        echo NL .'<p class="dot b">', lang(1505) ,'</p>'. NL .'<p>';
        if ($SIDU['eng'] != 'sqlite') echo cms_form('checkbox', 'db', $exp['db'], 'Use '.($SIDU['eng'] == 'mysql' ? 'DB' : 'Sch').' &nbsp; ');
        echo cms_form('checkbox', 'drop', $exp['drop'], lang(1506, $obj).' &nbsp; ');
        echo cms_form('checkbox', 'desc', $exp['desc'], lang(1507, $obj).' &nbsp; ');
        echo cms_form('checkbox', 'data', $exp['data'], array('list'=>lang(1508, $obj), 'class'=>'show', 'data-src'=>'#data')) ,'</p>';
    }

    $arr_clean = array(1=>'...\\\r...\\\n, \r\n, ...\\\n, \n » <i class="green">newLine</i> &nbsp;', '...\\\t, \t » <i class="green">TAB</i> &nbsp;', "...\\\', \' » <b class='green'>'</b> &nbsp;", '...\\\", \" » <b class="green">"</b> &nbsp;', '...\\\\, \\ » <b class="green">\\</b>');
    echo NL .'<p class="dot b">', lang(1509) ,'</p>'. NL .'<p>';
    echo cms_form('radio', 'ext', $exp['ext'], array('list'=>$arr_ext, 'class_cbox'=>' &nbsp; ', 'class'=>'expExt'));
    ...
    echo cms_form('text', 'sepR', $exp['sepR'], array('style'=>'width:50px')) ,' <i class="green">eg \n</i><br>';
    ...
    if ($mode != 'SQL' && !isset($exp['sql'][1])) {
        ...
        echo '</p>'. NL .'<p>where ', cms_form('text', 'where', $exp['where'], array('style'=>'width:300px')) ,'</p>';
    }
