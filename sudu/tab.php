...
function tab_desc($SIDU) {
    ...
    echo NL .'<pre>'. $desc . $comm . NL . NL . $idx;
    if ($SIDU['eng'] == 'mysql') {
        tab_desc_my_fk($SIDU);
    }
...
function my_fk_init_kids($sch, $tab, &$kids, &$fks) {
    ...
    SELECT
        ...
        TABLE_SCHEMA        AS kid_sch, TABLE_NAME        AS kid_tab, COLUMN_NAME        AS kid_col
    ...
    foreach ($rows as $row) {
        ...
        $kid_tab = $row['kid_tab'];
        $kid_col = $row['kid_col'];
        $fks[$kid_sch][$kid_tab][$sch][$tab][]  = $kid_col . ($col == $kid_col ? '' : ' : ' . $col);
        $kids[$sch][$tab][$kid_sch][$kid_tab][] = $col . ($col == $kid_col ? '' : ' : ' . $kid_col);
    }
}
        ...
        foreach ($tabs as $cur_tab => $fks) {
            ...
            if ($count > 1) {
                if (!isset($pool_colors[$sch_tab])) {
                    $pool_colors[$sch_tab] = count($pool_colors);
                }
