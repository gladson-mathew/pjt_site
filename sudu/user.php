<?php
$data = array('tab', 'userHost', 'user', 'host', 'pass', 'pass2', 'acs', 'acs2', 'grant', 'db');
...
function main($SIDU) {
    ...
    $colPass = substr($SIDU['dbL']->getAttribute(PDO::ATTR_SERVER_VERSION), 0, 4) == '5.7.' ? 'authentication_string' : 'Password';
...
function main_all_save($data, $opt) {
    ...
    elseif (count($data['acs']) == $opt['ttl'] - 1) $priv .= 'ALL PRIVILEGES';
    else {
...
function main_user_save($data, $colPass) {
    ...
    if ($data['pass'] != $data['pass2']) return lang(4727);
    $pass = sidu_val("select password('$data[pass]')");
    $where= "WHERE user='$data[user]' AND host='$data[host]' LIMIT 1";
    if (sidu_val('SELECT 1 FROM mysql.user '.$where)) sidu_run("UPDATE mysql.user SET $colPass='$pass' $where");
    else sidu_run("INSERT INTO mysql.user(Host,User,$colPass) VALUES('$data[host]','$data[name]','$pass')");
    $err = sidu_err(1);
    if ($err) return $err;
