...
function tab_init_pgsql(&$SIDU, $fk = null) {
    ...
    $tab = sidu_row("SELECT a.oid,a.relnamespace,a.relhasoids,obj_description(a.oid,'pg_class') AS comm\nFROM pg_class a,pg_namespace b WHERE a.relkind='$SIDU[3]' AND a.relnamespace=b.oid\nAND a.relname='$SIDU[4]' AND b.nspname='$SIDU[2]'");
...
function tab_desc_pgsql($SIDU, &$desc, &$comm, &$idx, &$help, $exp = 0) {
    ...
    $desc = substr($desc, 0, -1) . NL .') WITH (OIDS='. ($tab['relhasoids'] == 't' ? 'TRUE' : 'FALSE') .');';
